...
| LIVE-15 | Race tests, vet, module tidiness, snapshot checks, container isolation and pcap verification pass on one commit. | Exact-head GitHub Actions run. | PARTIAL — local tests are rerunning; the hosted workflow currently starts zero steps because no runner is assigned. |
...
for an exact-head run after lifecycle code changes.
