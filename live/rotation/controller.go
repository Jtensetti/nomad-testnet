...
func (config Config) recoverCompletedAttempt(epochNumber uint64) (resultMarker, bool, error) {
	...
	for _, entry := range entries {
		...
		isDiscardEvidence := false
		for _, ending := range []string{".discard.json", ".discard.json.pending", ".discard.json.erasing"} {
			if !strings.HasSuffix(suffix, ending) {
				continue
			}
			attemptText := strings.TrimSuffix(suffix, ending)
			attempt, err := strconv.Atoi(attemptText)
			if err != nil || attempt < 1 || attempt > 99 || fmt.Sprintf("%02d", attempt) != attemptText {
				return resultMarker{}, false, fmt.Errorf("unexpected DKG discard evidence %q", entry.Name())
			}
			info, err := os.Lstat(filepath.Join(config.StateRoot, entry.Name()))
			if err != nil || !info.Mode().IsRegular() {
				return resultMarker{}, false, fmt.Errorf("DKG discard evidence %q is not a regular file", entry.Name())
			}
			isDiscardEvidence = true
			break
		}
		if isDiscardEvidence {
			continue
		}
...
func loadMarker(path string) (resultMarker, error) {
	...
	if err := strictjson.RejectDuplicateKeys(encoded); err != nil {
		return resultMarker{}, fmt.Errorf("DKG result marker is ambiguous: %w", err)
	}
