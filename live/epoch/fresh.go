...
// It is intended for operator tooling that needs the fully verified descriptor
// in its original chain context (for example key erasure).
...
// FreshGuard is the production threshold-work guard. It deliberately refreshes
// the persisted epoch chain before every decision so a running share service
// sees a successor or halt written by another process without a restart.
...
// anything, performs the normal overwrite/unlink procedure, then fsyncs every
// affected parent directory before returning a signed statement. This closes
// two failure modes in the lower-level primitive: an accidentally wrong
// operator key must not destroy files and produce an unverifiable statement,
// and a crash must not be able to resurrect an unlink after an erasure
// statement has already been accepted as evidence.
...
// after lexical absolute-path normalization. Operator tooling uses this to
// prevent erasure arguments from deleting the epoch chain itself.
